<template>
	<view>
		...
		<view v-show="!parse_status">
			<uni-section title="快手视频直链解析" subTitle="使用快手的分享链接,解析高清无水印视频提供下载" type="line" padding>
				<uni-easyinput type="textarea" autoHeight v-model="share_url_text" placeholder="请输入快手分享链接">
				</uni-easyinput>
			</uni-section>
		...
		<view v-show="!parse_status">
			<uni-section title="使用说明" type="line">
				<uni-card :is-shadow="false">
					<text class="uni-body">这是一个快手无水印视频解析下载工具，将快手的分享链接粘贴在上方（无需去除文字，会自动去除），点击“开始解析”即可。</text>
				</uni-card>
			</uni-section>
		</view>
		...
		methods: {
			...
			parse() {
				if(this.share_url_text === "") {
					...
					if(this.share_url.substring(0, 28) !== "https://creater.eozatvmq.com") {
						uni.showToast({
							title: '这好像不是快手的分享链接耶~',
							icon: 'none',
							duration: 2000,
						...
						// 使用第三方接口(https://eeapi.cn/，密钥白嫖的，失效更换)
						uni.showLoading({
							title: "正在解析"
						});
						uni.request({
							method: 'GET',
							url: "https://eeapi.cn/api/video/3220C8646931BD306632D25A97EAC22015AE98BB73C68B8ED0/3/?url=" + this.share_url,
							header: {
							...
							success: (res) => {
								...
								this.video_url = res.data.data["down"];
								this.video_poster = res.data.data["cover"];
								this.video_name = res.data.data["title"];
